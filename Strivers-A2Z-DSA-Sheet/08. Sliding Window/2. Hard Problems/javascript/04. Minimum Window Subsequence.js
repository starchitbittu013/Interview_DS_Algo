...
- To check if s2 is a subsequence, we use two pointers:
  ...
  - If j reaches s2.length, we found s2 as a subsequence.
 ...
 * @return {string}
 */
function minWindowOptimal(s1, s2) {
    const n = s1.length;
    const m = s2.length;
...
Initial: i = 0, minLen = Infinity, startIndex = -1
...
start = end + 1 = 1
Window: s1[1..4] = "bcde", length = 5 - 1 = 4
minLen = 4, startIndex = 1
...
start = end + 1 = 5
Window: s1[5..8] = "bdde", length = 9 - 5 = 4
4 is NOT < minLen (4), don't update

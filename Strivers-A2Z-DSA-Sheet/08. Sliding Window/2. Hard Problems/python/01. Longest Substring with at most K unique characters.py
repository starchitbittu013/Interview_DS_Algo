...
1. Create a function, kDistinctChars, that takes 'k' and the input string 's' as parameters.
   - Initialize a dictionary, mp, to store the frequency of characters.
...
- Space complexity: O(k), as the space used by the dictionary is proportional to the number of distinct characters, which can be at most 'k'.
"""

def kDistinctChars(k: int, s: str) -> int:
    mp = {}
    start = 0
    ans = 0

...
1. Create a helper function, minimumSumOfSizeK, which finds the minimum sum of a subarray of size k.
   - Initialize ans to float('inf'), currsum to 0, and start to 0.
...
def minimumSumOfSizeK(arr: List[int], k: int) -> int:
    ans = float('inf')
    currsum = 0
    start = 0
...
def maxScore(cardPoints: List[int], k: int) -> int:
    ...
    discardedSum = minimumSumOfSizeK(cardPoints, windowSize)

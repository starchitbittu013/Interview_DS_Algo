...
function asteroidCollision(asteroids) {
    ...
    for (let i = 0; i < asteroids.length; i++) {
        const siz = Math.abs(asteroids[i]);
        while (st.length > 0 && (st[st.length - 1] > 0 && asteroids[i] < 0) && (st[st.length - 1] < siz)) {
            st.pop();
        }
        // Check if same size asteroids collide
        if (st.length > 0 && (st[st.length - 1] > 0 && asteroids[i] < 0) && st[st.length - 1] === siz) {
            st.pop();
        }
        else if (st.length === 0 || !(st[st.length - 1] > 0 && asteroids[i] < 0)) {
            st.push(asteroids[i]);
        }
    }
    return st;

...
function sumSubarrayMins(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        const leftElements = BigInt(i - prevS[i]);
        const rightElements = BigInt(nextS[i] - i);
        // this formula is arrived by mathematical calculation
        ans += ((leftElements % mod) * (rightElements % mod) * (BigInt(arr[i]) % mod)) % mod;
    }
...
module.exports = { prevSmaller, nextSmaller, sumSubarrayMins };

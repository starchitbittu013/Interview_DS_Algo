...
def asteroid_collision(asteroids: List[int]) -> List[int]:
    ...
    for i in range(len(asteroids)):
        siz = abs(asteroids[i])
        while st and (st[-1] > 0 and asteroids[i] < 0) and (st[-1] < siz):
            st.pop()
        # Check if same size asteroids collide
        if st and (st[-1] > 0 and asteroids[i] < 0) and st[-1] == siz:
            st.pop()
        elif not st or not (st[-1] > 0 and asteroids[i] < 0):
...
if __name__ == "__main__":
    ...
    print(f"Output: {asteroid_collision(asteroids)}")

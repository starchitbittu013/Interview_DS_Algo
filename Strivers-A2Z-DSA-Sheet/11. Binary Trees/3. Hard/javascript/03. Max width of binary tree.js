...
function widthOfBinaryTree(root) {
    ...
    let ans = 0n;
    const queue = [];
    queue.push({ index: 0n, node: root });

    while (queue.length > 0) {
        const n = queue.length;
        const start = queue[0].index;
        const end = queue[queue.length - 1].index;

        for (let i = 0; i < n; i++) {
            const { index: idx, node: curr } = queue.shift();
            // note we are doing -start because we don't need previous value
            const index = idx - start;

            if (curr.left) queue.push({ index: 2n * index + 1n, node: curr.left });
            if (curr.right) queue.push({ index: 2n * index + 2n, node: curr.right });
        }

...
To indicate that the current node is not a Sum Tree, we return a special value (e.g., -Infinity) from the recursive function. If the result is -Infinity, then the binary tree is not a Sum Tree. Otherwise, it is a Sum Tree.
...
function solve(root) {
    ...
    return (lef + rig === root.data) ? 2 * root.data : -Infinity;
}
